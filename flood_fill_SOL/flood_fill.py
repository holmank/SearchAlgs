def flood_fill(image, start, newColor):
    ...
    expanded = set()
    while len(queue)!=0:
        currentLocation = queue.pop(0) # changing this changes BFS/DFS
        row, col = currentLocation
        if currentLocation not in expanded:
            setPixel(image, row, col, newColor)
            expanded.add(currentLocation)
            neighbors = getNeighbors(image, row, col)
            for neighbor in neighbors:
                neighborRow, neighborCol = neighbor
                if getPixel(image, neighborRow, neighborCol)==initialColor:
